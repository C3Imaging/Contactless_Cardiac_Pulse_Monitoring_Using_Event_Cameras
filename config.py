...
_C.TRAIN.DATA.PREPROCESS.FS = 30
...
_C.VALID.DATA.PREPROCESS.FS = 30
...
def update_config(config, args):
    ...
    #config.TRAIN.DATA.CACHED_PATH = os.path.join(config.TRAIN.DATA.CACHED_PATH, config.TRAIN.DATA.EXP_DATA_NAME+'_train')
    ...
    if not config.TEST.USE_LAST_EPOCH and config.VALID.DATA.DATASET is not None:
        ...
        #config.VALID.DATA.CACHED_PATH = os.path.join(config.VALID.DATA.CACHED_PATH, config.VALID.DATA.EXP_DATA_NAME+'_valid')
    ...
    #config.TEST.DATA.CACHED_PATH = os.path.join(config.TEST.DATA.CACHED_PATH, config.TEST.DATA.EXP_DATA_NAME+'_test')
    ...
    if not ext: # no file extension
        config.SIGNAL.DATA.FILE_LIST_PATH = os.path.join(config.SIGNAL.DATA.FILE_LIST_PATH, \
                                                         ...
                                                         config.SIGNAL.DATA.DATASET + '_' + \
                                                         str(config.SIGNAL.DATA.BEGIN) + '_' + \
                                                         str(config.SIGNAL.DATA.END) + '.csv')
    elif ext != '.csv':
        raise ValueError('SIGNAL dataset FILE_LIST_PATH must either be a directory path or a .csv file name')

    if ext == '.csv' and config.SIGNAL.DATA.DO_PREPROCESS:
        raise ValueError('User specified SIGNAL dataset FILE_LIST_PATH .csv file already exists. \
                         Please turn DO_PREPROCESS to False or delete existing SIGNAL dataset FILE_LIST_PATH .csv file.')
