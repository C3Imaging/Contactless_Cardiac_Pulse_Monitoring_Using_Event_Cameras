...
class BaseLoader(Dataset):
    ...
    def face_resize(self, frames, width, height):
        # Frame Resizing
    ...
    @staticmethod
    def process_resample(signal, frames_ts, signal_ts):
        """Apply Pre-processing"""
        #inversion
        flipped_ecg = [-1*x for x in signal]
        #smoothing
        smoothed = savgol_filter(filtered, 1000, 2, 0)
        #filtering
        ...
        for i in range(len(frames_ts)):
            frame_ts = frames_ts[i]
            while j<len(clipped):
                if signal_ts[j]<frame_ts:
                    j+=1
                    continue
                else:
                    break
