...
def _calculate_peak_hr(ppg_signal, fs):
    ...
    ppg_peaks, _ = scipy.signal.find_peaks(ppg_signal, distance= (fs/2.5) )
    hr_peak = 60 / (np.mean(np.diff(ppg_peaks)) / fs)
    return hr_peak

def calculate_metric_per_video_ev(predictions, labels, fs=30, diff_flag=True, use_bandpass=True, hr_method='FFT'):
